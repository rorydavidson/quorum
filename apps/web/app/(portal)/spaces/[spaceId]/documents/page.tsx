...
import { getSpaceFiles, getAuthoredDocuments, getUserFromHeaders } from '@/lib/api-client';
...
export default async function SpaceDocumentsPage({ params, searchParams }: Props) {
  ...
  let unfiledDocs: PortalDocument[] = [];
  let error: string | null = null;

  try {
    const [spaceData, allDocs] = await Promise.all([
      getSpaceFiles(spaceId, cookie, folderId),
      getAuthoredDocuments(spaceId, cookie).catch(() => [] as PortalDocument[]),
    ]);
    data = spaceData;
    unfiledDocs = allDocs.filter((d) => !d.sectionId);
  } catch (err) {
        ...
        <>
          <div className="mb-3 flex items-center justify-between">
            <p className="text-xs text-snomed-grey/50">
              ...
              {unfiledDocs.length > 0 && ` · ${unfiledDocs.length} authored`}
            </p>
          </div>
          <DocumentList spaceId={spaceId} files={data.files} canUpload={canUpload} canCreateOfficialRecord={isAdmin} />
          {unfiledDocs.length > 0 && (
            <SectionAuthoredDocs documents={unfiledDocs} spaceId={spaceId} />
          )}
        </>
      )}
    </div>
